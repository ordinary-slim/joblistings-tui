...
def query(search_term, location, results_wanted=20):
    jobs = scrape_jobs(
         site_name=jobsites,
...
def fakequery(search_term, location, results_wanted=20):
    import pandas as pd
    csvfile = "c++-fem-france-2026-04-03-13:00.csv"
    df = pd.read_csv(csvfile)
    print(f"Loaded {len(df)} jobs from {csvfile} for the search term '{search_term}' in {location}.")
    return df
